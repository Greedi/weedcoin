...
// Copyright (c) 2012-2013 weedcoin Developers
...
// for both bitcoind and Weedcoin-qt, to make it harder for attackers to
// target servers or GUI users specifically.
const std::string CLIENT_NAME("Weedice");
...
const string strMessageMagic = "weedcoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // weedcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // weedcoin: 1 days for diff retarget
static const int64 nTargetSpacing = 5 * 60; // weedcoin: 5 minutes between blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // weedcoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal Weedcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "weedcoin", wxOK | wxICON_EXCLAMATION);
...
bool LoadBlockIndex(bool fAllowNew)
{
	...
	//  vMerkleTree: d14936e84c 
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 13/jul/2012 Greedi, WeedCoin developer";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ffff001d01042f4e592054696d65732031332f6a756c2f32303132204772656564692c2057656564436f696e20646576656c6f706572") << OP_CHECKSIG;
        CBlock block;
...
unsigned char pchMessageStart[4] = { 0xf5, 0xb6, 0xc4, 0x78 }; // weedcoin: increase each by adding 2 to Weedcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "addr")
    {
        ...
        // weedcoin: a temporary fix for the spam.
        if (vAddr.size() == 499)
        {
            pfrom->Misbehaving(100);
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    CRITICAL_BLOCK(cs_mapTransactions)
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // weedcoind: Reduce the exempted free transactions to 500 bytes (from Weedcoin's 3000 bytes)
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    while (fGenerateBitcoins)
    {
        ...
        printf("Running WeedcoinMiner with %d transactions in block\n", pblock->vtx.size());
